...
Tile::Tile(int textureID, const sf::Vector2f &worldPos) :
    ...
    mWorldPos(worldPos),
    mFadeCurrent(0.0f), mFadeMax(1.0f), mVisible(false),
    mOccupant(nullptr)
{
...
void Tile::reduceFadeCurrent(float deltaVal)
{
    ...
    if (mOccupant)
        mOccupant->reduceFadeCurrent(deltaVal);

    if (mFadeCurrent == 0.0f)
        mVisible = false;
    else
        mVisible = true;
}
...
void Tile::draw(sf::RenderTarget & target, sf::RenderStates states) const
{
    ...
    for (auto i : mItems)
    {
        // Modify the items before drawing then change back
        // to allow inventories to draw them properly
        sf::Vector2f origin(Constants::Items::HalfWidth, -Constants::Items::HalfHeight);
        sf::Vector2f scale(Constants::Items::WidthRatio, Constants::Items::HeightRatio);
